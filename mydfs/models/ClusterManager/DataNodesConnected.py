...
class DataNodesConnected:
  def __init__(self):
    ...
    self.__LAST_UPDATE_THRESHOLD = 45 # nanoseconds
    self.__CPU_USAGE_THRESHOLD = 70 # percent
    self.__MINIUM_SHARD_TO_STORE = 8

  def __remove_dead_data_nodes(self):
    tokens_to_remove = [token for token, data_node in self.__data_nodes.items() if data_node.time_since_last_update() > self.__LAST_UPDATE_THRESHOLD * 1e9]
  ...
  def get_suitable_data_nodes_for_upload(self) -> list[str]:
    ...
    dn = [token for token, data_node in self.__data_nodes.items() if
          ...
          and data_node.__cpu_usage <= self.__CPU_USAGE_THRESHOLD
